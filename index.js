...
program
  .command("here")
  .alias("init")
  .option("--body-parser-json <enabled>", "Enable express json body parser. Options: enabled, disabled, prompt.",
    parseED,
    true)
  .option("--body-parser-raw <enabled>", "Enable express raw body parser. Options: enabled, disabled, prompt.",
    parseED,
    "prompt")
  .option("--body-parser-urlencoded <enabled>",
    "Enable express urlencoded body parser. Options: enabled, extended, disabled, prompt.",
    parseED,
    "prompt")
  .option("-p, --port <port>", "Application Port", 3500)
  .option("-s, --skip-npm-install", "Skip npm install")
  .option("-f, --force", "Create a package.json if it does not exists")
